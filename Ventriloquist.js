...
exports.Change = function Change(client, msg) {
  if (msg.author.id !== Globals.g_GrooveID) { msg.channel.send('LOL nope, this only for Groove'); } else {
    const gData = msg.content.split(' ');
    const iChannelID = gData[1];
    if (iChannelID) {
      Globals.g_VentriloquistOutputChannelID = iChannelID;
      msg.channel.send(`Changed output channel to: ${Globals.g_VentriloquistOutputChannelID}`);
    }
  }
};
